...
if(opt$performance_measures) {
  ...
  bsub_command <- paste0('bsub -e ', opt$directory,'logs/',opt$out_prefix, '_performance_measure_fillout.err -n 2 -R "rusage[mem=8]" -W 0:59 "Rscript ',opt$script,'performance_measure_script.R -g ', ground_file_name,' -t ', test_file_name, ' -d ',opt$directory,' -s ',name_test,' -n ',name_ground, ' -c ', opt$directory,' -m  -p  -o fillout_', opt$out_prefix)
